...
- importing a workflow file is no longer limited to native Numel JSON: the interop path now also accepts a pragmatic subset of n8n workflow JSON, including common HTTP, branch, switch, merge, simple wait, and portable code-node patterns, and converts it into a runnable Numel graph with explicit warnings anywhere manual cleanup is still needed
