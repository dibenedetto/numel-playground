...
| Spaces / workflow metadata | **Git + sqlite metadata** through `GitSpaceStore` + `DbGitSpaceProvider`. **Fully working**. Repo-like spaces, history, refs, and forking are real here. | **Git + PostgreSQL metadata** using the same product model. **Fully working**. The same repo-like spaces, history, refs, and forking should remain visible at the app surface. |
